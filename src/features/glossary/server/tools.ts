  ...
  return {
    upsertGlossary: tool({
      ...
      execute: (input: UpsertGlossaryInput) =>
        Effect.gen(function* () {
          return yield* GlossaryService.use((svc) => svc.upsertGlossary({ workspaceId, ...input }));
    ...
    listGlossary: tool({
      ...
      execute: () =>
        Effect.gen(function* () {
          return yield* GlossaryService.use((svc) => svc.listGlossary(workspaceId));
        }).pipe(appRuntime().runPromise),
    }),
    deleteGlossary: tool({
      description:
      ...
      execute: (input: DeleteGlossaryInput) =>
        Effect.gen(function* () {
          yield* GlossaryService.use((svc) => svc.deleteGlossary(input.termId, workspaceId));
          return { deleted: true };
        }).pipe(appRuntime().runPromise),
    }),
  };
