  ...
  return {
    createLesson: tool({
      ...
      execute: (input: CreateLessonInput) =>
        Effect.gen(function* () {
          ...
          return yield* service.createTitled("lesson")({ workspaceId, title: input.title, content: input.content });
    ...
    createRecord: tool({
      ...
      execute: (input: CreateRecordInput) =>
        Effect.gen(function* () {
          const service = yield* ArtifactService;
          return yield* service.createTitled("record")({ workspaceId, title: input.title, content: input.content });
        }).pipe(appRuntime().runPromise),
    }),
    createReference: tool({
      ...
      execute: (input: CreateReferenceInput) =>
        Effect.gen(function* () {
          const service = yield* ArtifactService;
          return yield* service.createTitled("reference")({ workspaceId, title: input.title, content: input.content });
        }).pipe(appRuntime().runPromise),
    }),
    listReferences: tool({
      description:
    ...
    deleteLesson: tool({
      ...
      execute: (input: DeleteInput) =>
        Effect.gen(function* () {
          const service = yield* ArtifactService;
          const deleted = yield* service.deleteTitled("lesson")(input.id, workspaceId);
          return { deleted };
        }).pipe(appRuntime().runPromise),
    }),
    deleteRecord: tool({
      ...
      execute: (input: DeleteInput) =>
        Effect.gen(function* () {
          const service = yield* ArtifactService;
          const deleted = yield* service.deleteTitled("record")(input.id, workspaceId);
          return { deleted };
        }).pipe(appRuntime().runPromise),
    }),
    deleteReference: tool({
      ...
      execute: (input: DeleteInput) =>
        Effect.gen(function* () {
          const service = yield* ArtifactService;
          const deleted = yield* service.deleteTitled("reference")(input.id, workspaceId);
          return { deleted };
        }).pipe(appRuntime().runPromise),
    }),
  };
