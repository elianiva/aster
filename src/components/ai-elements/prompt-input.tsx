...
export const PromptInputAttachments = ({ className, ...props }: PromptInputAttachmentsProps) => {
  ...
  const referencedSources = useContext(LocalReferencedSourcesContext);

  if (files.length === 0 && !referencedSources?.sources.length) {
    return null;
  }
      ...
      <Attachments variant="grid">
        ...
        {referencedSources?.sources.map((source) => (
          <Attachment
            ...
            className="size-20"
            data={source}
            onRemove={() => referencedSources.remove(source.id)}
          >
            <AttachmentPreview />
            <AttachmentRemove />
          </Attachment>
